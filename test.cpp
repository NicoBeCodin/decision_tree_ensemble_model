...
#include <vector>
#include <string>
#include "functions.h"  // Inclure le fichier header contenant openCSV
...
/* Pas de test pour printStringCSV */
...
TEST(ConvertToIntTest, InvalidInputs) {
    ...
    EXPECT_THROW(convertToInt("123.45"), std::invalid_argument);
...
TEST(ConvertToFloatTest, InvalidInputs) {
    ...
    EXPECT_THROW(convertToFloat("123,45"), std::invalid_argument); // Virgule au lieu de point
...
TEST(ProcessParametersCSVTest, ValidCSV) {
    ...
    EXPECT_EQ(result[0][2], 3.3);

    EXPECT_EQ(result[1][0], 4);
    EXPECT_EQ(result[1][1], 5);
    EXPECT_EQ(result[1][2], 6.6);

    EXPECT_EQ(result[2][0], 7);
    EXPECT_EQ(result[2][1], 8);
    EXPECT_EQ(result[2][2], 9.9);

    // Supprime le fichier après le test
    deleteTestCSV(filename);
}

TEST(ProcessParametersCSVTest, EmptyCSV) {
    // Crée un fichier CSV vide
...
/* Test de la fonction printParamAndResults */

TEST(printParamAndResultsTest, ValidCSV) {
    // Crée deux fichiers, un contenant les en-têtes et l'autre contenant les paramètres (avec la dernière colonne concernant la performance)
    const std::string filename_header = "HeaderTest";
    const std::string filename_param = "ParametersTest";

    createTestCSV(filename_header, "param1, param2, param3");
    createTestCSV(filename_param, "1,2,3\n4,5,6\n7,8,9");

    // Lire les fichier avec OpenCSV
    vector<string> content_header = openCSV(filename_header);
    vector<vector<string>> content_param = openCSV(filename_param);
    
    // On appelle processParametersCSV et process
}
