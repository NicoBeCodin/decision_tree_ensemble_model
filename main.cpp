...
int main() {
    try {
        ...
        if (X.empty() || y.empty()) {
            throw std::runtime_error("Erreur: Impossible de lire les données ou fichier vide.");
        }

        // Validation de la taille des données
        if (X.size() < 2) {
            throw std::runtime_error("Erreur: Jeu de données trop petit pour l'entraînement et le test.");
        }
        ...
        std::cout << "Choisissez la méthode à utiliser:\n";
        std::cout << "1: Arbre de régression\n";
        std::cout << "2: Bagging\n";
        std::cout << "3: Boosting\n";
        std::cout << "4: Arbre de décision simple\n";
        
        int choice;
        while (!(std::cin >> choice) || choice < 1 || choice > 4) {
            std::cin.clear();
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            std::cout << "Erreur: Veuillez entrer un nombre entre 1 et 4.\n";
        }
