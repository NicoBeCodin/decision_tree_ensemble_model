...
//To complile: g++ -o tests_bagging ../ensemble_bagging/bagging.cpp ../functions_tree/decision_tree_single.cpp ../functions_tree/math_functions.cpp ../ensemble_boosting/loss_function.cpp tests_bagging.cpp -std=c++17
//This creates a lot of trees so to clean use : rm bagging_model.txt_tree_**
...
void testBagging() {
    ...
    Bagging model(50, 3, 2, 0.01, std::move(loss_function));

    // Train the model
    model.train(X_flattened, rowLength, y, 0);
    ...
    // Save the model
    model.save("bagging_model.txt");

    // Load the model
    Bagging loadedModel(10, 3, 2, 0.01, std::make_unique<MSELoss>());
    loadedModel.load("bagging_model.txt");
